...
- `ffi.rs` / `ffi_doc.rs` / `ffi_filters.rs` / `ffi_agent.rs` / `ffi_assistant.rs` / `ffi_style.rs` / `ffi_color.rs` — thin shims only; shared plumbing lives in `ffi_util.rs`, never copy-pasted.
