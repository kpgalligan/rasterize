...
fn rzdc_version_1_files_still_load_without_masks() {
    ...
    let future = dir.path().join("v5.rzdc");
    std::fs::write(&future, &v5).unwrap();
    let err = RzDocument::open(future.to_str().unwrap())
        .err()
        .expect("a future version must be refused");
    assert!(err.contains("unsupported RZDC version 5"), "got: {err}");
...
fn rzdc_v3_round_trips_clipped_flags() {
    ...
    assert_eq!(
        u32::from_le_bytes(bytes[4..8].try_into().unwrap()),
        4,
        "the writer writes the current version (clipping bumped it to 3, layer styles to 4)"
    );
