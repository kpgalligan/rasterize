...
fn rzdc_version_1_files_still_load_without_masks() {
    ...
    let future = dir.path().join("v9.rzdc");
    std::fs::write(&future, &v9).unwrap();
    let err = RzDocument::open(future.to_str().unwrap())
        .err()
        .expect("a future version must be refused");
    assert!(err.contains("unsupported RZDC version 9"), "got: {err}");
...
fn rzdc_v3_round_trips_clipped_flags() {
    ...
    assert_eq!(
        ...
        "the writer writes the current version (clipping bumped it to 3, layer \
         ...
         locks to 7, guides and the ruler origin to 8)"
    );
