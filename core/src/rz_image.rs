...
//! atomic save, and the per-format encoders. (Named `rz_image`, not `image`,
//! to avoid ambiguity with the `image` crate.)
...
impl RzImage {
    ...
    pub(crate) fn save(&self, path: &str, format: Format, jpeg_quality: u8) -> Result<(), String> {
        save_atomically(path, |tmp_path| {
            let file =
                File::create(tmp_path).map_err(|e| format!("failed to create {path}: {e}"))?;
            let mut writer = BufWriter::new(file);
            self.encode_into(&mut writer, format, jpeg_quality)?;
            writer
                .flush()
...
/// (`RzImage::save` here, `RzDocument::save_native` in `rzdc`). `write`
